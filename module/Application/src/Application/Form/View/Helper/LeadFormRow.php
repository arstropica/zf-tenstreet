...
class LeadFormRow extends FormRow
{
	...
	public function render (ElementInterface $oElement, $labelPosition = null)
	{
		...
		if ($oElement->getMessages()) {
			...
			if (($sInputErrorClass = $this->getInputErrorClass()) != false) {
				if (($sElementClass = $oElement->getAttribute('class')) != false) {
					if (! preg_match(
							'/(\s|^)' . preg_quote($sInputErrorClass, '/') .
									 '(\s|$)/', $sElementClass)) {
						$oElement->setAttribute('class', 
								trim($sElementClass . ' ' . $sInputErrorClass));
					}
				} else {
		...
		if (($sColumSize = $oElement->getOption('column-size')) &&
				 $sLayout !== LeadFilterForm::LAYOUT_HORIZONTAL) {
		...
		if (($sElementType === 'submit' || $sElementType === 'button' ||
				 $sElementType === 'reset') &&
				 $sLayout === LeadFilterForm::LAYOUT_INLINE) {
			return $sElementContent . PHP_EOL;
		}
	...
	protected function renderLabel (ElementInterface $oElement)
	{
		if (($sLabel = $oElement->getLabel()) &&
			...
			$sLabel = $oTranslator->translate($sLabel, 
	...
	protected function renderElement (ElementInterface $oElement)
	{
		...
		if (($sLabelContent = $this->renderLabel($oElement)) != false) {
			/*
			...
			if (($oElement instanceof Button) or ($oElement instanceof Submit)) {
				...
				switch ($sLayout) {
					...
					case LeadFilterForm::LAYOUT_INLINE:
						if ($sElementType !== 'checkbox') {
							if (empty($aLabelAttributes['class'])) {
								$aLabelAttributes['class'] = 'sr-only';
							} elseif (! preg_match('/(\s|^)sr-only(\s|$)/', 
									$aLabelAttributes['class'])) {
								$aLabelAttributes['class'] = trim(
										$aLabelAttributes['class'] . ' sr-only');
							}
					...
					case LeadFilterForm::LAYOUT_HORIZONTAL:
						if ($sElementType !== 'checkbox') {
							if (empty($aLabelAttributes['class'])) {
								$aLabelAttributes['class'] = 'control-label';
							} else {
								if (! preg_match('/(\s|^)control-label(\s|$)/', 
										$aLabelAttributes['class'])) {
									$aLabelAttributes['class'] = trim(
											$aLabelAttributes['class'] .
													 ' control-label');
				...
				$sLabelOpen = $oLabelHelper->openTag(
						$oElement->getAttribute('id') ? $oElement : $aLabelAttributes);
				$sLabelClose = $oLabelHelper->closeTag();
				
				// Allow label html escape desable
				// $sLabelContent =
				// $this->getEscapeHtmlHelper()->__invoke($sLabelContent);
				
				if (! $oElement instanceof LabelAwareInterface ||
						 ! $oElement->getLabelOption('disable_html_escape')) {
					$sLabelContent = $this->getEscapeHtmlHelper()->__invoke(
							$sLabelContent);
				}
			}
		}
		...
		if ($this->requiredFormat && $oElement->getAttribute('required') &&
		...
		switch ($sLayout) {
			...
			case LeadFilterForm::LAYOUT_INLINE:
				...
				// Checkbox elements are a special case, element is already
				// rendered into label
				if ($sElementType === 'checkbox') {
					$sElementContent = sprintf(self::$checkboxFormat, 
							$sElementContent);
				} else {
					if ($this->getLabelPosition() === self::LABEL_PREPEND) {
						$sElementContent = $sLabelOpen . $sLabelContent .
								 $sLabelClose . $sElementContent;
					} else {
						$sElementContent = $sElementContent . $sLabelOpen .
								 $sLabelContent . $sLabelClose;
					}
				}
				...
				if ($this->renderErrors) {
					$sElementContent .= $this->getElementErrorsHelper()->render(
							$oElement);
				}
			...
			case LeadFilterForm::LAYOUT_HORIZONTAL:
				...
				// Column size
				if (($sColumSize = $oElement->getOption('column-size')) != false) {
					$sClass .= ' col-' . $sColumSize;
				}
				
				// Checkbox elements are a special case, element is rendered
				// into label
				if ($sElementType === 'checkbox') {
					return sprintf(self::$horizontalLayoutFormat, $sClass, 
							sprintf(self::$checkboxFormat, $sElementContent));
				}
				
				if ($this->getLabelPosition() === self::LABEL_PREPEND) {
					return $sLabelOpen . $sLabelContent . $sLabelClose . sprintf(
							self::$horizontalLayoutFormat, $sClass, 
							$sElementContent);
				} else {
					return sprintf(self::$horizontalLayoutFormat, $sClass, 
							$sElementContent) . $sLabelOpen . $sLabelContent .
	...
	protected function renderHelpBlock (ElementInterface $oElement)
	{
		return ($sHelpBlock = $oElement->getOption('help-block')) ? sprintf(
				...
				$this->getEscapeHtmlHelper()->__invoke(
						($oTranslator = $this->getTranslator()) ? $oTranslator->translate(
								$sHelpBlock, $this->getTranslatorTextDomain()) : $sHelpBlock)) : '';
	}
}