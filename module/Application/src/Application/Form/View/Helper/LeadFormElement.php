...
class LeadFormElement extends FormElement implements TranslatorAwareInterface
{
	...
	protected $classMap = array(
			...
			'Application\Form\Element\DateRange' => 'formDateRange'
	);
	...
	public function render (ElementInterface $oElement)
	{
			...
			if ($sElementClass) {
				if (! preg_match('/(\s|^)form-control(\s|$)/', $sElementClass)) {
					$oElement->setAttribute('class', 
							trim($sElementClass . ' form-control'));
				}
			} else {
		...
		if ($aAddOnAppend || $aAddOnPrepend) {
			...
			return sprintf(self::$inputGroupFormat, trim($sSpecialClass), 
					$sMarkup);
		}
		return $sMarkup;
	}
	...
	protected function renderAddOn ($aAddOnOptions)
	{
		...
		if ($aAddOnOptions instanceof ElementInterface) {
			...
			throw new InvalidArgumentException(
					sprintf(
							...
							is_object($aAddOnOptions) ? get_class(
									$aAddOnOptions) : gettype($aAddOnOptions)));
		}
		...
		if (! empty($aAddOnOptions['text'])) {
			...
			if (! is_scalar($aAddOnOptions['text'])) {
				throw new InvalidArgumentException(
						sprintf(
								...
								is_object($aAddOnOptions['text']) ? get_class(
										$aAddOnOptions['text']) : gettype(
										$aAddOnOptions['text'])));
			} elseif ($oTranslator) {
				$sMarkup .= $oTranslator->translate($aAddOnOptions['text'], 
			...
			if (is_array($aAddOnOptions['element']) || ($aAddOnOptions['element'] instanceof Traversable &&
					 ! ($aAddOnOptions['element'] instanceof ElementInterface))) {
				$oFactory = new Factory();
				$aAddOnOptions['element'] = $oFactory->create(
				...
				throw new LogicException(
						sprintf(
								...
								is_object($aAddOnOptions['element']) ? get_class(
										$aAddOnOptions['element']) : gettype(
										$aAddOnOptions['element'])));
			}
			
			$aAddOnOptions['element']->setOptions(
					array_merge($aAddOnOptions['element']->getOptions(), 
							array(
									'disable' => true
							)));
			...
			if ($aAddOnOptions['element'] instanceof Button) {
				...
				// Element contains dropdown, so add-on container must be a
				// "div"
		...
		return sprintf(self::$addonFormat, $sAddonTagName, trim($sAddonClass), 
				$sMarkup, $sAddonTagName);
	}
	 ...
	 *        	: [optional] translator. Default is null, which sets no
	 *        	translator.
	 * @param string $sTextDomain
	 *        	: [optional] text domain Default is null, which skips
	 *        	setTranslatorTextDomain
	 * @return LeadFormElement
	 */
	public function setTranslator (TranslatorInterface $oTranslator = null, 
			$sTextDomain = null)
	{
